...
async function getApplicationsByResumeId(resumeId, userId) {
  ...
  const applications = await ApplicationRecord.findAll({
    ...
    order: [['apply_date', 'DESC'], ['created_at', 'DESC']]
  });
...
async function getApplicationStats(userId) {
  ...
  // 获取最近和首次投递日期
  const latestApplyDate = applications.length > 0
    ? applications[0].apply_date
    : null;

  const firstApplyDate = applications.length > 0
    ? applications[applications.length - 1].apply_date
    : null;
