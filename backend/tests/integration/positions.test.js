...
describe('目标岗位管理集成测试', () => {
  ...
  describe('创建岗位测试', () => {
    ...
    test('应该拒绝空字符串name', async () => {
      await expect(
        positionService.createPosition(testUser1.id, '')
    ...
    test('应该拒绝超过100字符的name', async () => {
      ...
      await expect(
        positionService.createPosition(testUser1.id, tooLongName)
  ...
  describe('查询岗位测试', () => {
    ...
    test('应该正确统计岗位关联的简历数量', async () => {
      ...
      const result = await positionService.getPositionById(
        position.id,
        testUser1.id,
        true
      );
    ...
    test('应该拒绝获取不存在的岗位', async () => {
      ...
      await expect(
        positionService.getPositionById(fakeId, testUser1.id)
    ...
    test('应该拒绝访问其他用户的岗位', async () => {
      ...
      // 用户2尝试访问
      await expect(
        positionService.getPositionById(position.id, testUser2.id)
      ).rejects.toThrow('无权限访问该目标岗位');
    });
  ...
  describe('更新岗位测试', () => {
    ...
    test('应该拒绝更新为与其他岗位重复的name', async () => {
      // 创建两个岗位
      const position1 = await positionService.createPosition(testUser1.id, '前端开发');
  ...
  describe('删除岗位测试', () => {
    ...
    test('应该拒绝删除不存在的岗位', async () => {
      ...
      await expect(
        positionService.deletePosition(fakeId, testUser1.id)
      ).rejects.toThrow('目标岗位不存在');
    });
    ...
    test('应该拒绝删除其他用户的岗位', async () => {
      ...
      // 用户2尝试删除
      await expect(
        positionService.deletePosition(position.id, testUser2.id)
      ).rejects.toThrow('无权限删除该目标岗位');
    });
    ...
    test('应该拒绝删除包含简历的岗位', async () => {
      ...
      // 尝试删除
      await expect(
        positionService.deletePosition(position.id, testUser1.id)
      ).rejects.toThrow(/该岗位下还有.*个简历版本，无法删除/);
    });
    ...
    test('应该在错误消息中显示正确的简历数量', async () => {
      ...
      await expect(
        positionService.deletePosition(position.id, testUser1.id)
      ).rejects.toThrow('该岗位下还有3个简历版本，无法删除');
    });
  ...
  describe('完整CRUD流程测试', () => {
    test('应该完成创建-查询-更新-删除的完整流程', async () => {
      ...
      // 2. 查询岗位详情
      const fetched = await positionService.getPositionById(
        created.id,
        testUser1.id
      );
      ...
      // 4. 再次查询验证更新
      const refetched = await positionService.getPositionById(
        created.id,
        testUser1.id
      );
      ...
      // 5. 删除岗位
      const deleteResult = await positionService.deletePosition(
        created.id,
        testUser1.id
      );
      ...
      // 6. 验证删除后无法查询
      await expect(
        positionService.getPositionById(created.id, testUser1.id)
      ).rejects.toThrow('目标岗位不存在');
    });

    test('应该正确处理多个岗位的复杂操作流程', async () => {
      // 创建多个岗位
      const pos1 = await positionService.createPosition(testUser1.id, '前端开发');
  ...
  describe('多用户隔离测试', () => {
    test('应该确保不同用户的岗位完全隔离', async () => {
      ...
      const user1Pos2 = await positionService.createPosition(testUser1.id, '后端开发');

      // 用户2创建岗位
      const user2Pos1 = await positionService.createPosition(testUser2.id, '产品经理');
      const user2Pos2 = await positionService.createPosition(testUser2.id, '项目经理');
      ...
      // 验证用户1无法访问用户2的岗位
      await expect(
        positionService.getPositionById(user2Pos1.id, testUser1.id)
      ).rejects.toThrow('无权限访问该目标岗位');

      // 验证用户2无法访问用户1的岗位
      await expect(
        positionService.getPositionById(user1Pos1.id, testUser2.id)
      ).rejects.toThrow('无权限访问该目标岗位');
      ...
      // 验证用户2无法删除用户1的岗位
      await expect(
        positionService.deletePosition(user1Pos1.id, testUser2.id)
      ).rejects.toThrow('无权限删除该目标岗位');
    });
